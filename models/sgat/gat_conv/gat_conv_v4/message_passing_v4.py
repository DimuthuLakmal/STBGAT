...
class MessagePassingV4(torch.nn.Module):
    ...
    def __init__(
            ...
            aggr_original: Optional[Union[str, List[str], Aggregation]] = "mean",
            *,
            aggr_kwargs: Optional[Dict[str, Any]] = None,
            aggr_original_kwargs: Optional[Dict[str, Any]] = None,
        ...
        # aggr for skip connection
        if aggr_original is None:
            self.aggr_original = None
            self.aggr_original_module = None
        elif isinstance(aggr_original, (str, Aggregation)):
            self.aggr_original = str(aggr_original)
            self.aggr_original_module = aggr_resolver(aggr_original, **(aggr_original_kwargs or {}))
        ...
        self.inspector.inspect(self.message_original)
        self.inspector.inspect(self.aggregate, pop_first=True)
        self.inspector.inspect(self.aggregate_original, pop_first=True)
        self.inspector.params['aggregate'].pop('aggr', None)
        self.inspector.params['aggregate_original'].pop('aggr', None)
        self.inspector.inspect(self.message_and_aggregate, pop_first=True)
        self.inspector.inspect(self.update, pop_first=True)
        self.inspector.inspect(self.update_original, pop_first=True)
        self.inspector.inspect(self.edge_update)

        self._user_args = self.inspector.keys(
            ['message', 'message_original', 'aggregate', 'aggregate_original', 'update', 'update_original']).difference(
            self.special_args)
    ...
    def reset_parameters(self):
        ...
        if self.aggr_original_module is not None:
            self.aggr_original_module.reset_parameters()
    ...
    def propagate_skip_connection(self, edge_index: Adj, size: Size = None, **kwargs):
        r"""The initial call to start propagating messages.
        ...
        if is_sparse(edge_index) and self.fuse and not self.explain:
            ...
            for i in range(decomposed_layers):
                ...
                msg_original_kwargs = self.inspector.distribute('message_original', coll_dict)
                out_skip_connection = self.message_original(**msg_original_kwargs)

                aggr_original_kwargs = self.inspector.distribute('aggregate_original', coll_dict)
                out_skip_connection = self.aggregate_original(out_skip_connection, **aggr_original_kwargs)

                update_original_kwargs = self.inspector.distribute('update_original', coll_dict)
                out_skip_connection = self.update_original(out_skip_connection, **update_original_kwargs)

                if decomposed_layers > 1:
                    decomp_out.append(out_skip_connection)

            if decomposed_layers > 1:
                out_skip_connection = torch.cat(decomp_out, dim=-1)
    ...
    def aggregate_original(self, inputs: Tensor, index: Tensor,
                           ptr: Optional[Tensor] = None,
                           dim_size: Optional[int] = None) -> Tensor:
        r"""Aggregates messages from neighbors as
        :math:`\bigoplus_{j \in \mathcal{N}(i)}`.
        ...
        as specified in :meth:`__init__` by the :obj:`aggr` argument.
        """
        return self.aggr_original_module(inputs, index, ptr=ptr, dim_size=dim_size,
                                         dim=self.node_dim)
    ...
    def update_original(self, inputs: Tensor) -> Tensor:
        r"""Updates node embeddings in analogy to
        :math:`\gamma_{\mathbf{\Theta}}` for each node
        :math:`i \in \mathcal{V}`.
        Takes in the output of aggregation as first argument and any argument
