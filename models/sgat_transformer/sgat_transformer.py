...
class SGATTransformer(nn.Module):
    def __init__(
            ...
            transformer_features: int = 5,  # number of encoders
        ...
        if transformer_lookup_index:
            self.lookup_idx_enc, max_lookup_len_enc = create_lookup_index(merge=True if transformer_features == 1 else False)
        ...
        self.encoders = nn.ModuleList([
            ...
            for _ in range(transformer_features)
        ])
        ...
        self.decoder = TransformerDecoder(input_dim=transformer_input_dim,
                                          ...
                                          n_heads=transformer_n_heads,
                                          dropout=transformer_dropout,
                                          enc_features=transformer_features,
                                          sgat_settings=sgat_settings,
                                          merge_embed=transformer_merge_emb,
    ...
    def forward(self, x, graph_x, y=None, graph_y=None, train=True):
        ...
        enc_outs = torch.zeros((self.transformer_features, x.shape[0] * x.shape[2], x.shape[1], emb_dim)).to(self.device)

        for idx, encoder in enumerate(self.encoders):
            x_i = x[:, :, :, idx: idx + 1] if x is not None else None
            graph_x_i = graph_x[idx] if graph_x is not None else None
            lookup_idx_i = self.lookup_idx[idx] if self.transformer_features > 1 else self.lookup_idx_enc
